  ...
  return (
    <>
      ...
      <aside
        ...
        className={`
          flex flex-col border-r border-border-subtle bg-bg-sidebar transition-all duration-200 ease-out
      ...
      >
        ...
        <div className="flex h-14 items-center gap-3 border-b border-border-subtle px-4">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-accent-gold/15">
            <svg xmlns="http://www.w3.org/2000/svg" width={16} height={16} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" className="text-accent-gold" aria-hidden="true">
        ...
        {/* Navigation */}
        <nav aria-label="Sidebar navigation" className="flex-1 space-y-1 overflow-y-auto px-2 py-4">
          {sidebarItems.map((item) => {
            ...
            return (
              ...
              >
                <span className="shrink-0" aria-hidden="true">{iconMap[item.icon]}</span>
                {sidebarOpen && (
        ...
        <div className="hidden border-t border-border-subtle p-2 lg:block">
          <button
            onClick={toggleSidebar}
            className="flex w-full items-center justify-center rounded-lg px-3 py-2 text-text-muted transition-colors hover:bg-white/5 hover:text-text-secondary focus-visible:ring-2 focus-visible:ring-accent-gold focus-visible:outline-none"
